...
class ObjectOrientation extends CodeTaskSuite("Basic Object Orientation", 10) {
  koan(""" Object Orientation:
    ...
    class RationalNumber(val numerator: Int, val denominator: Int) {
      def print(): String = numerator + "/" + denominator
    ...
    r1.print should be("2/3")

    val r2 = new RationalNumber(1, 2)
    r1.multiply(r2).print should be("2/6")
    (r1 multiply r2).print should be("2/6")
  }
  ...
  codetask(""" Exercises: (Class implementation)
    ...
    Sadly the code of the has been assigned to a member of the class which was not private. Right there we
