...
class Ex22_CaseClasses extends CodeTaskSuite("Case Classes",22) {
  ...
  koan("Case classes can have mutable properties") {
    case class Dog(var name: String, breed: String) // the field name is specifically marked as mutable with var, now you can rename a dog, but change its breed? nah!
    val d1 = Dog("Scooby", "Doberman")
